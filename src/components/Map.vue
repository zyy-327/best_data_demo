...
<script>
import '../../public/static/map/world_fix'
export default {
	data() {
		return {
			// 实例对象
	...
	methods: {
		...
		updateChart() {
			...
			for (let i = 0; i < data.length; i++) {
				...
				// 如果有对应的国家名字就把data里面的名字和数据添加到新数组中
				if (geoCoord) {
					res.push({
						name: data[i].name,
						// concat数组拼接 把data里面的数据拼接到geoCoord里面 res数组里就有name 和value值了
